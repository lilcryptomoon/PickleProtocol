PickleProtocol Core version *0.15.1* is now available from:

  <https://download.pickleprotocol.org/pickleprotocol-0.15.1/>
...
A number of changes to the way PickleProtocol Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 spears per kB (0.00001 PCK). This is to prepare for dropping the minimum transaction fee to 0.1 spears per kB (0.0001 PCK) in the next release.

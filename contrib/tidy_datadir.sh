...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete PickleProtocol database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no PickleProtocol datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old PickleProtocol datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected PickleProtocol 0.7 datadir."
    ;;
  3)
    echo "Detected PickleProtocol pre-0.8 datadir."
    ;;
  4)
    echo "Detected PickleProtocol 0.8 datadir."
    ;;
esac

...
const char * const BITCOIN_CONF_FILENAME = "pickleprotocol.conf";
const char * const BITCOIN_PID_FILENAME = "pickleprotocold.pid";
...
#else
    const char* pszModule = "pickleprotocol";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PickleProtocol";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PickleProtocol";
#else
    // Unix
    return pathRet / ".pickleprotocol";
#endif
#endif
}
